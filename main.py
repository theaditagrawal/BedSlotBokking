...
from flask import render_template # Adjust import based on your app structure
from sqlalchemy import text

@app.route('/hospitalbedstats')
def hospitalbedstats():
    try:
        # SQL query with nested query, aggregate query, and join
        query = text("""
            SELECT h.hname, 
                   h.hcode, 
                   SUM(h.normalbed) AS total_normal_beds,
                   SUM(h.hicubed) AS total_hicu_beds,
                   SUM(h.icubed) AS total_icu_beds,
                   SUM(h.vbed) AS total_ventilator_beds,
                   (SELECT COUNT(*) 
                    FROM bookingpatient b 
                    WHERE b.hcode = h.hcode) AS total_booked_beds
            FROM hospitaldata h
            GROUP BY h.hname, h.hcode

            UNION ALL

            SELECT 'Total' AS hname, 
                   NULL AS hcode, 
                   SUM(normalbed) AS total_normal_beds,
                   SUM(hicubed) AS total_hicu_beds,
                   SUM(icubed) AS total_icu_beds,
                   SUM(vbed) AS total_ventilator_beds,
                   (SELECT COUNT(*) 
                    FROM bookingpatient) AS total_booked_beds
            FROM hospitaldata
        """)
        ...
        return render_template('hospitalbedstats.html', results=result)
